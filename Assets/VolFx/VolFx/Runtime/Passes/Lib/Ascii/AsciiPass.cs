...
namespace VolFx
{
    ...
    public class AsciiPass : VolFx.Pass
    {
        ...
        private static readonly int s_MappingTex = Shader.PropertyToID("_MappingTex");
        
        private static float        s_RefResolution = 720;
        ...
        public override bool Validate(Material mat)
        {
            ...
            mat.SetVector(s_Data, new Vector4(settings.m_Impact.value, settings.m_Solid.value));
            ...
            if (_absolute)
                scale *= s_RefResolution / (float)Screen.height;
            ...
            mat.SetVector(s_ImageColor, imageCol);
            
            _validateNoise();
            mat.SetTexture(s_NoiseTex, _noiseTex);
            settings.m_Mapping.value.GetTexture(ref _mappingTex, 0f, .99f);
            mat.SetTexture(s_MappingTex, _mappingTex);
            ...
            _noiseMad.x = _noiseScale * settings.m_Noise.value;
            _noiseMad.y = _noiseScale * settings.m_Noise.value;
            ...
            /*void validatePrint(bool isOn)
            {
                if (mat.IsKeywordEnabled("USE_PRINT") == isOn)
                    return;
                
                if (isOn) mat.EnableKeyword("USE_PRINT");
                else      mat.DisableKeyword("USE_PRINT");
            }*/
        ...
        protected override void _editorSetup(string folder, string asset)
        {
            ...
            if (_gradient == null)
                _gradient = UnityEditor.AssetDatabase.LoadAssetAtPath<Texture2D>($"{folder}{sep}Data{sep}Gradient{sep}ascii-grad-motif.png");
