...
#pragma warning restore CS0168
            ...
            if (m_List == null || bug)
            {
                ...
                m_List.drawHeaderCallback = rect =>
                {
                    ...
                    if (Event.current.type == EventType.MouseDown && Event.current.button == 1 && rect.Contains(Event.current.mousePosition))
                    {
                        ...
                        {
                            var objects = propertyList.GetList().ToList();
                            
                            foreach (var sp in objects)
                            {
                                AssetDatabase.RemoveObjectFromAsset(sp.objectReferenceValue);
                                Object.DestroyImmediate(sp.exposedReferenceValue);
                            }
                            
                            propertyList.ClearArray();
                            propertyList.serializedObject.ApplyModifiedProperties();
#if !UNITY_2022_3_OR_NEWER
                            AssetDatabase.SaveAssets();
#endif
    ...
    {
        public static void AddItem(SerializedProperty soc, ScriptableObject so)
        {
            var list = soc.FindPropertyRelative("m_List");
            list.arraySize ++;
            AssetDatabase.AddObjectToAsset(so, soc.serializedObject.targetObject);
            
            list.GetArrayElementAtIndex(list.arraySize - 1).objectReferenceValue = so;
            soc.serializedObject.ApplyModifiedPropertiesWithoutUndo();
